...
library(MASS)
library(matlib)
load(file = "mcshapiro.test.Rdata")
...
beta_sample <- mvrnorm(n=1, mu=b0, Sigma = B0)

iter <- 1000 #Nb of iterations
for (k in seq(1,iter)){
  for (i in seq(1, n)){
    W_given_b[i] <- pgdraw(N[i],psi(X,i,beta_sample))
  }
  V <- V_omega(W_given_b)
  m <- m_omega(V)
  beta_sample <- mvrnorm(n=1,m,V)
}
